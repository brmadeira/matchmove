...
void hp_reconstruct_points(HomologPoint *hp, Cameras *c, double tol_error)
{
 ...
 {
  int local_max_ninliers = 0, nframes = hp->last_frame - hp->first_frame + 1;
  gsl_vector *best_point = gsl_vector_alloc(3);
  Boolean best_inliers[MAX_FRAMES_PER_FEATURE];

  #pragma omp parallel for private(j) schedule(dynamic,8)
  for(j = i + 1; j <= hp->last_frame; j++)
  {
   gsl_matrix *pi = gsl_matrix_alloc(3,4);
   gsl_matrix *pj = gsl_matrix_alloc(3,4);
   gsl_vector *pt1 = gsl_vector_alloc(2);
   gsl_vector *pt2 = gsl_vector_alloc(2);
   gsl_vector *point = gsl_vector_alloc(3);
   ...
   int ninliers = hp_ninliers(hp, point, inliers, c, tol_error);

   #pragma omp critical
   {
    if(ninliers > local_max_ninliers)
    {
     local_max_ninliers = ninliers;
     gsl_vector_memcpy(best_point, point);
     memcpy(best_inliers, inliers, sizeof(inliers));
    }
   }
  ...
  if(local_max_ninliers > hp->ninliers)
  {
   gsl_vector_memcpy(hp->p, best_point);
   hp->ninliers = local_max_ninliers;
   adjust_inliers(hp, best_inliers);
  }

  gsl_vector_free(best_point);
 }
}
