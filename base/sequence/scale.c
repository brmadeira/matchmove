...
#define SCALE_MAX_ITERATIONS 100
...
{
 int i, n = 0;
 SubSeqNode *aux, *table[MAX_NUMBER_OF_SUBSEQ] ; 
 ...
 for( i = 0; i < n; i++ ) 
     scale_vector[i] = get_scale(table[i]);
}
...
double get_scale( SubSeqNode *s )
{
 ...
 int best_ninliers = 0, ninliers, i, k;
 ...
 for( scale = MIN_INTERFRAG_SCALE; scale < 100.; scale *= SCALE_OPTIMIZE_RADIUS ){
      if( (ninliers = scale_reproj_inliers( reproj_inliers, error, scale, s, p1, p2 )) < best_ninliers)
          break;
      global_error = mean_global_error( error, reproj_inliers );
    
      if( global_error <= min_global_error ){
 ...
 if( best_ninliers < MIN_ACCEPTABLE_NINLIERS ){
     printf( "Calibration Fail!! Repeat Operation\n" );
     exit(0);     
 }
...
static int scale_reproj_inliers( int *reproj_inliers, gsl_vector *error, double scale, SubSeqNode *s, int *p1, int *p2 )
{
 ...
 for( i = 0; i < error->size; i++ ){
  ...
  if( w > MIN_FRAC_SCALE_TRHLD*(s->next->nframes) ){
     ninliers++; 
