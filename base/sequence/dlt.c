...
static double matrix33_distance( gsl_matrix *k, gsl_matrix *kref );
static void reconstruct( SubSeqNode *s, gsl_matrix *k );
static double projs_distance( SubSeqNode *s, int frame1, int frame2 );
...
{
  int i, j, frame1, frame2, ninliers=0, ransac_ninliers, max_ninliers = -1;
  int *inliers, *ransac_inliers, *max_inliers, patience = 0, prev_max_ninliers = -1;
  ...
  for( i = s->nframes - 1; (i > 0) && (max_ninliers < ACCEPTANCE_NINLIERS ) 
                           && (max_ninliers < s->nfeatures )
                           && (patience < NINLIERS_PATIENCE); i-=RECONSTRUCT_FRAME_STEP ){
    printf( "Processing (%i, *): %i\n", i, max_ninliers );
    prev_max_ninliers = max_ninliers;
    for( j = 0; (j < i) && (max_ninliers < ACCEPTANCE_NINLIERS); j++ ) {
      if( projs_distance( s, i, j ) > MIN_MOV_DIST2 ){
    ...
    if( max_ninliers == prev_max_ninliers ){
       patience++;
       printf( "patience %i\n", patience );  
    }
    else
...
double matrix33_distance( gsl_matrix *k, gsl_matrix *kref )
{
 double e = 0;
 int i, j;
...
double projs_distance( SubSeqNode *s, int frame1, int frame2 )
{
 gsl_vector *v1, *v2;
 int i, ninliers = 0;
 double d = 0;
